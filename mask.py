...
def create_mask(image_path, pathdatasetoutput = None):
    pic_path = image_path
    mask_path = "./img/blue-mask2.png"
    show = False
    model = "hog"
    FaceMasker(pic_path, mask_path, show, model).mask(pathdatasetoutput)
...
class FaceMasker:
    ...
    def mask(self, pathdatasetoutput = None):
        import face_recognition
    ...
    def _save(self, pathdatasetoutput = None):
        if pathdatasetoutput is not None:
            path_splits = os.path.splitext(self.face_path)
            cartellaImmagine = path_splits[0].split("/", 11)[-1]
            real_pathdatasetoutput = pathdatasetoutput + "/" + cartellaImmagine + path_splits[1]
            #print(real_pathdatasetoutput)
            try:
                os.makedirs(pathdatasetoutput + "/" + cartellaImmagine.rsplit("/", 1)[0])
            except FileExistsError:
                pass
            self._face_img.save(real_pathdatasetoutput)
            print(f'Save to {real_pathdatasetoutput}')
        else:
            path_splits = os.path.splitext(self.face_path)
            #new_face_path = path_splits[0] + '-with-mask' + path_splits[1]
            appoggio = path_splits[0]
            split = appoggio.rsplit("/",2)
            new_face_path = split[0] + "/001-with-mask/" + split[2] + path_splits[1]
